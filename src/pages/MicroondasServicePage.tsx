...
import { getMicroondasServiceBySlug } from '@/data/microondas-services';
...
const MicroondasServicePage = () => {
  const { slug } = useParams<{ slug: string }>();
  const service = slug ? getMicroondasServiceBySlug(slug) : undefined;

  if (!service) return <Navigate to="/micro-ondas" replace />;
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href={`https://assistenciatecnica.maringa.br/micro-ondas/${service.slug}`} />
          ...
          <div className="bg-muted/50 border-b border-border/50">
            <div className="container mx-auto px-4 py-3">
              <nav className="flex items-center gap-2 text-sm flex-wrap">
                ...
                <ChevronRight className="w-4 h-4 text-muted-foreground" />
                <Link to="/micro-ondas" className="text-muted-foreground hover:text-primary transition-colors">Micro-ondas</Link>
                <ChevronRight className="w-4 h-4 text-muted-foreground" />
...
export default MicroondasServicePage;
