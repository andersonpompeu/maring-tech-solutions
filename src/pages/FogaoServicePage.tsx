...
import { getFogaoServiceBySlug } from '@/data/fogao-services';
...
const FogaoServicePage = () => {
  const { slug } = useParams<{ slug: string }>();
  const service = slug ? getFogaoServiceBySlug(slug) : undefined;

  if (!service) return <Navigate to="/fogao" replace />;
          ...
          <div className="bg-muted/50 border-b border-border/50">
            <div className="container mx-auto px-4 py-3">
              <nav className="flex items-center gap-2 text-sm flex-wrap">
                ...
                <ChevronRight className="w-4 h-4 text-muted-foreground" />
                <Link to="/fogao" className="text-muted-foreground hover:text-primary transition-colors">Fogão</Link>
                <ChevronRight className="w-4 h-4 text-muted-foreground" />
...
export default FogaoServicePage;
