...
import { getMaquinaDeLavarServiceBySlug } from '@/data/maquina-de-lavar-services';
...
const MaquinaDeLavarServicePage = () => {
  const { slug } = useParams<{ slug: string }>();
  const service = slug ? getMaquinaDeLavarServiceBySlug(slug) : undefined;

  if (!service) return <Navigate to="/maquina-de-lavar" replace />;
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href={`https://assistenciatecnica.maringa.br/maquina-de-lavar/${service.slug}`} />
          ...
          <div className="bg-muted/50 border-b border-border/50">
            <div className="container mx-auto px-4 py-3">
              <nav className="flex items-center gap-2 text-sm flex-wrap">
                ...
                <ChevronRight className="w-4 h-4 text-muted-foreground" />
                <Link to="/maquina-de-lavar" className="text-muted-foreground hover:text-primary transition-colors">Máquina de Lavar</Link>
                <ChevronRight className="w-4 h-4 text-muted-foreground" />
...
export default MaquinaDeLavarServicePage;
