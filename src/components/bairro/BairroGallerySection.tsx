...
interface BairroGallerySectionProps {
  bairro: string;
}
...
const BairroGallerySection = ({ bairro }: BairroGallerySectionProps) => {
          ...
          {galleryItems.map((item) => (
            ...
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className={`absolute inset-0 bg-gradient-to-t from-primary/90 via-primary/40 to-transparent transition-opacity duration-300 ${hoveredId === item.id ? 'opacity-100' : 'opacity-0'}`} />
                <div className={`absolute top-4 right-4 w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-300 ${hoveredId === item.id ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}>
                  <ZoomIn className="w-5 h-5 text-white" />
                </div>
                <div className="absolute top-4 left-4">
                  <span className="bg-secondary text-secondary-foreground text-xs font-semibold px-3 py-1 rounded-full">{item.category}</span>
                </div>
                <div className={`absolute bottom-0 left-0 right-0 p-5 transition-all duration-300 ${hoveredId === item.id ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                  <h3 className="text-lg font-heading font-bold text-white mb-1">{item.title}</h3>
                  <p className="text-white/80 text-sm line-clamp-2">{item.description}</p>
                </div>
              </div>
              <div className={`p-4 transition-all duration-300 ${hoveredId === item.id ? 'opacity-0 h-0 p-0' : 'opacity-100'}`}>
                <h3 className="font-heading font-semibold text-foreground mb-1 truncate">{item.title}</h3>
        ...
        <div className="text-center mt-12">
          <p className="text-muted-foreground mb-4">
           ...
           <a
             ...
             className="inline-flex items-center gap-2 bg-whatsapp text-whatsapp-foreground font-semibold px-8 py-3 rounded-full hover:bg-whatsapp/90 transition-all duration-300 hover:-translate-y-1 shadow-whatsapp"
           >
        ...
        <DialogContent className="max-w-4xl w-full p-0 bg-background border-none overflow-hidden">
          {selectedImage && (
            <div className="relative">
              <button onClick={() => setSelectedImage(null)} className="absolute top-4 right-4 z-50 w-10 h-10 bg-background/80 backdrop-blur-sm rounded-full flex items-center justify-center text-foreground hover:bg-background transition-colors">
                <X className="w-5 h-5" />
              </button>
              <button onClick={() => navigateImage('prev')} className="absolute left-4 top-1/2 -translate-y-1/2 z-50 w-12 h-12 bg-background/80 backdrop-blur-sm rounded-full flex items-center justify-center text-foreground hover:bg-background transition-colors">
                <ChevronLeft className="w-6 h-6" />
              </button>
              <button onClick={() => navigateImage('next')} className="absolute right-4 top-1/2 -translate-y-1/2 z-50 w-12 h-12 bg-background/80 backdrop-blur-sm rounded-full flex items-center justify-center text-foreground hover:bg-background transition-colors">
              ...
              <div className="p-6 bg-background">
                <span className="bg-secondary text-secondary-foreground text-xs font-semibold px-3 py-1 rounded-full">{selectedImage.category}</span>
                <h3 className="text-xl font-heading font-bold text-foreground mb-2 mt-3">{selectedImage.title}</h3>
...
export default BairroGallerySection;